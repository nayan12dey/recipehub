...
export default function DashboardOverview() {
    ...
    return (
        ...
        <div className="min-h-full bg-transparent transition-colors duration-300">
            ...
            <div className="mb-8">
                <div className="flex items-center gap-3 mb-1">
                    <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-orange-500 to-red-500 flex items-center justify-center shadow-sm shadow-orange-200 dark:shadow-none">
                        <FaChartLine size={14} className="text-white" />
                    </div>
                ...
                <p className="text-gray-500 dark:text-gray-400 text-sm ml-11">
                    Welcome back,{" "}
                    <span className="font-semibold text-orange-600 dark:text-orange-400">{firstName}</span>!
                    Here&apos;s a snapshot of your activity.
                </p>
            </div>
                ...
                {statCards.map(
                    ({
                        ...
                        <div
                            id={id}
                            key={id}
                            className={`relative rounded-2xl border p-6 shadow-md overflow-hidden group transition-all duration-200 hover:-translate-y-1
                        ${borderColor} ${bgLight} ${shadowColor}
                        dark:bg-gray-900 dark:border-gray-800/70 dark:shadow-black/20`}
                        >
                            {/* Decorative circles */}
                            <div
                                className={`absolute -top-6 -right-6 w-24 h-24 rounded-full bg-gradient-to-br ${gradient} opacity-10 dark:opacity-5`}
                            />
                            <div
                                className={`absolute -bottom-8 -left-4 w-20 h-20 rounded-full bg-gradient-to-br ${gradient} opacity-10 dark:opacity-5`}
                            />
                            ...
                            {/* Value */}
                            {loading ? (
                                <div className="h-10 w-20 rounded-lg bg-gray-200 dark:bg-gray-800 animate-pulse mb-1" />
                            ) : (
                                <p className={`text-4xl font-extrabold leading-none mb-1 tabular-nums ${textColor} dark:text-gray-50`}>
                                    {value.toLocaleString()}
                                </p>
                            )}

                            {/* Label */}
                            <p className="text-gray-800 dark:text-gray-400 font-semibold text-sm">{label}</p>

                            {/* Badge */}
                            <p className={`text-[11px] font-medium mt-1 ${badgeColor} dark:brightness-110`}>
                                {badgeText}
                            </p>
                        </div>
                    )
            ...
            {/* ── Activity Summary Banner ── */}
            <div className="rounded-2xl bg-gradient-to-br from-orange-500 to-red-500 p-6 shadow-lg shadow-orange-200 dark:shadow-none relative overflow-hidden">
                <div className="absolute -top-8 -right-8 w-32 h-32 rounded-full bg-white/10" />
                <div className="absolute -bottom-6 -left-6 w-24 h-24 rounded-full bg-white/10" />
                ...
                <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p className="text-orange-50 dark:text-orange-100 text-sm mt-1 max-w-sm leading-relaxed">
                            You&apos;ve created{" "}
                            <span className="font-bold text-white">
                                {stats.totalRecipes ?? 0}
                            </span>{" "}
                    ...
                    <a
                        href="/dashboard/user/add-recipe"
                        className="shrink-0 inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-orange-600 text-sm font-bold hover:bg-orange-50 active:scale-95 transition-all duration-200 shadow-sm"
                    >
                        <FaUtensils size={13} />
                        Add New Recipe
                    </a>
